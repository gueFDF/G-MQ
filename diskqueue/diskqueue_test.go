...
func TestDiskQueueRoll(t *testing.T) {
	...
	ml := int64(len(msg))
	dq := New(dqName, tmpDir, 10*(ml+4), int32(ml), 1<<10, 2500, 2*time.Second)
	defer dq.Close()
	NotNil(t, dq)
	Equal(t, int64(0), dq.Depth())
...
func TestDiskQueueEmpty(t *testing.T) {
	...
	Equal(t, dq.(*diskQueue).readFileNum, dq.(*diskQueue).nextReadNum)
...
func TestDiskQueueResize(t *testing.T) {
	...
	ml := int64(len(msg))
	dq := New(dqName, tmpDir, 8*(ml+4), int32(ml), 1<<10, 2500, time.Second)
	NotNil(t, dq)
	Equal(t, int64(0), dq.Depth())
	...
	dq.Close()
	dq = New(dqName, tmpDir, 10*(ml+4), int32(ml), 1<<10, 2500, time.Second)
